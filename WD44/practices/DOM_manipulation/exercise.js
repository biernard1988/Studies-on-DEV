...
const inventors = [
  ...
  /*   const q2 = inventors.filter((inventor) => { */
    // console.log(inventor.categories);
    // if (inventor.categories) {
    //   if (inventor.categories[1] === "mathematician") {
    //     return true;
    //   }
    // } else {
    //   return false;
    // }
    // return inventor.categories && inventor.categories[1] === "mathematician";
    // using optional chaining "?." ( it checks if the key exist if yes then continue checking the next keys )
    
    // return inventor.categories?.[1] === "mathematician";
/*   });
...
const q3 = inventors.filter((inventor) => {
    if (inventor.categories?.includes("physicist") && inventor.categories?.includes("man")) {
      return true;
    }
    return false;
  })
  ...
  const q4 = inventors.map((inventor) => {
  ...
  const q5 = inventors.map((inventor) => {
  ...
  const q7 = inventors.reduce((totalYears, inventor) => {
    return totalYears + (inventor.passed - inventor.year);

  },0);

  console.log("question7", q7);
  ...
  const q11 = data.reduce((acumulator, vehicles) => {
    if (!acumulator[vehicles]) {
      acumulator[vehicles] = 1;
    } else {
      acumulator[vehicles] += 1;
    }
    return acumulator;
